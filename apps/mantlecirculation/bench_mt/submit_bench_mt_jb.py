...
# Outputs live on scratch ($HOME quota is tight on JUWELS).
SCRATCH_ROOT = Path("/p/scratch/walberlamovinggeo/boehm2/mantlecirculation/bench_mt")
JOB_DIR      = SCRATCH_ROOT / "jobs_jb"
OUTPUT_ROOT  = SCRATCH_ROOT / "outputs_jb"
...
RADIAL_LAT_LEVEL     = 8
RADIAL_LAT_SDR       = 1
# JUWELS variant: anchor *every* diagonal at base_gpus = 4 = 1 node, varying the
# starting radial level instead. fill 126 -> rad_start=8 (isotropic), 63 -> 7,
# 32 -> 6, 16 -> 5, 8 -> 4. subdom/GCD stays 10 along each diagonal (rad_sdr
# grows by 1 each step, alongside rad_level and n_gpus).
RADIAL_DEFAULT_FILLS = [126, 63, 32, 16, 8]
RADIAL_BASE_GPUS     = 4   # 1 node on JUWELS Booster
RADIAL_BASE_RADSDR   = 0   # base_gpus = 4^lat_sdr * 2^rad_sdr -> 4*1 with lat_sdr=1

def radial_start_offset(fill_mdofs: int) -> int:
    """Radial extra levels at the 4-GCD anchor that hit `fill_mdofs` per GCD."""
    return round(math.log2(fill_mdofs * 1e6 * RADIAL_BASE_GPUS
                            / DOFS_PER_LEVEL[RADIAL_LAT_LEVEL]))

# Kept for compatibility with any caller that imports radial_base.
def radial_base(fill_mdofs: int) -> tuple[int, int]:
    return RADIAL_BASE_GPUS, RADIAL_BASE_RADSDR
...
# JUWELS Booster: each node has 4 A100 GPUs and 48 cores. `booster` caps single
# jobs at 384 nodes; jobs above that need `largebooster` (same hardware, different
# policy).
GPUS_PER_NODE    = 4
...
def write_lumi_job(cell_tag: str, out_path: Path, nodes: int, tpn: int,
    ...
    """Per-cell sbatch script for one benchmark_operators cell on JUWELS Booster.
    (Name kept for parity with the upstream LUMI driver so the radial/iso cell
    builders below call into here unchanged.)"""
...
def render_job_script(level: int, n_gpus: int, executions: int, warmup: int) -> tuple[Path, Path, dict]:
    ...
    echo_line = (f"Cell: {cell_tag}  level={level}  sdr={sdr}  subdomains={subdomains_for(sdr)}  "
                 f"executions={executions}  n_gpus={n_gpus}  nodes={nodes}x{tpn}  partition={partition_for(nodes)}")
