...
size_t team_shmem_size_dn_wave() const
{
    ...
    const size_t nscalars = size_t( nxy ) * 3          //coords_sh
...
KOKKOS_INLINE_FUNCTION void run_team_fast_dirichlet_neumann_wave( const Team& team ) const
{
    ...
    /* Load the shared elements.
     *  Note that we load all shared data at once, although they are not defined on different
     *  domains. */
    ...
    Kokkos::parallel_for( Kokkos::ThreadVectorRange( team, TOTAL_PAIRS ), [&]( int t ) {
        ...
        if ( rr <= hex_rad_ )
        {
            ...
            if ( node == 0 )
            {
                //Thread 0 also loads the shared array.
        ...
        for ( int block = 0; block < kWaveBlocksPerWave; ++block )
        {
            ...
            for ( int w = 0; w < 2; ++w )
            {
                ...
                double g0, g1, g2;
                {
                    ...
                    if ( active )
                    {
                        const int knid = WEDGE_NODE_OFF[w][node_in_cell][0] + 2 * WEDGE_NODE_OFF[w][node_in_cell][1];
                    ...
                    g0 = inv_det * ( ( J_1_1 * J_2_2 - J_1_2 * J_2_1 ) * gx + ( -J_1_0 * J_2_2 + J_1_2 * J_2_0 ) * gy +
                                     ( J_1_0 * J_2_1 - J_1_1 * J_2_0 ) * gz );
                    g1 = inv_det * ( ( -J_0_1 * J_2_2 + J_0_2 * J_2_1 ) * gx + ( J_0_0 * J_2_2 - J_0_2 * J_2_0 ) * gy +
                                     ( -J_0_0 * J_2_1 + J_0_1 * J_2_0 ) * gz );
                    g2 = inv_det * ( ( J_0_1 * J_1_2 - J_0_2 * J_1_1 ) * gx + ( -J_0_0 * J_1_2 + J_0_2 * J_1_0 ) * gy +
                ...
                if constexpr ( !Diagonal )
                {
                    const bool node_in_range =
                        active && ( node_in_cell >= cmb_shift ) && ( node_in_cell < 6 - surface_shift );
                ...
                {
                    const bool node_in_range =
                        active && ( node_in_cell >= surface_shift ) && ( node_in_cell < 6 - cmb_shift );
                    if ( node_in_range )
                    {
