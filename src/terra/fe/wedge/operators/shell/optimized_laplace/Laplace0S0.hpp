...
class Laplace0S0
{
  ...
  public:
    ...
    KOKKOS_INLINE_FUNCTION void operator()( const Team& team ) const
    {
        ...
        ScalarT* shmem_coords = (ScalarT*) team.team_shmem().get_shmem( 4 * 3 * sizeof( ScalarT ) );
        ...
        {
            dense::Vec< ScalarT, 3 > p1, p2, p3;
            const int                egdew = 1 - wedge;
            for ( int d = 0; d < 3; ++d )
            {
                p1( d ) = shmem_coords[6 * wedge + 3 * wedge + d];
                p2( d ) = shmem_coords[6 * egdew + 3 * wedge + d];
                p3( d ) = shmem_coords[6 * wedge + 3 * egdew + d];
            }
            ...
            for ( int q = 0; q < QuadRule::num_quad_points; q++ )
            {
                ...
                const dense::Mat< ScalarT, 3, 3 > J                = jac( p1, p2, p3, r_1, r_2, xi, eta, zeta );
                const ScalarT                     det              = J.det();
                const ScalarT                     abs_det          = Kokkos::abs( det );
                const dense::Mat< ScalarT, 3, 3 > J_inv_transposed = J.inv_transposed( det );
...
static_assert( linalg::OperatorLike< Laplace0S0< float, quadrature::QuadRuleWedge1Point< float > > > );
static_assert( linalg::OperatorLike< Laplace0S0< float, quadrature::QuadRuleWedge6Points< float > > > );
static_assert( linalg::OperatorLike< Laplace0S0< double, quadrature::QuadRuleWedge1Point< double > > > );
static_assert( linalg::OperatorLike< Laplace0S0< double, quadrature::QuadRuleWedge6Points< double > > > );
