...
class Laplace1S2
{
  ...
  private:
    ...
    Kokkos::View< ScalarT*** [2][QuadRule::num_quad_points][3][3], Kokkos::LayoutRight > g1_;

    // subdomain, x, y, 2 wedges, quad points
    Kokkos::View< ScalarT*** [2][QuadRule::num_quad_points], Kokkos::LayoutRight > g2_;
  ...
  public:
    ...
    KOKKOS_INLINE_FUNCTION void operator()( const Team& team ) const
    {
        ...
        {
            const int q               = i % N_q;
            const int wedge           = i / N_q;
            shmem_g3[wedge * N_q + q] = g2_( local_subdomain_id, x_cell, y_cell, wedge, q );

            dense::Mat< ScalarT, 3, 3 > b_inv_t;
            for ( int d1 = 0; d1 < 3; ++d1 )
            {
                for ( int d2 = 0; d2 < 3; ++d2 )
                {
                    b_inv_t( d1, d2 ) = g1_( local_subdomain_id, x_cell, y_cell, wedge, q, d1, d2 );
                }
            }
            ScalarT xi, eta, zeta;
            QuadRule::get_quad_point( q, xi, eta, zeta );
        ...
        // dense::Vec< ScalarT, 3 > quad_points[QuadRule::num_quad_points];
        //
        // QuadRule::quad_points( quad_points );
        ...
        // TODO: we could simply compare the r value to the min/max radius of the domain
        ...
        const dense::Vec< ScalarType, 3 >* grad_phy ) const
    {
        ...
        // TODO: this could be simplified to
        // constexpr int offset[2][6] = { { 0, 1, 2, 4, 5, 6 }, { 3, 2, 1, 7, 6, 5 } };
...
static_assert( linalg::OperatorLike< Laplace1S2< float, quadrature::QuadRuleWedge1Point< float > > > );
static_assert( linalg::OperatorLike< Laplace1S2< float, quadrature::QuadRuleWedge6Points< float > > > );
static_assert( linalg::OperatorLike< Laplace1S2< double, quadrature::QuadRuleWedge1Point< double > > > );
static_assert( linalg::OperatorLike< Laplace1S2< double, quadrature::QuadRuleWedge6Points< double > > > );
