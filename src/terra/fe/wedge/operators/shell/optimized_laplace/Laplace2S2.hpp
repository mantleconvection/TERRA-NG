...
class Laplace2S2
{
  ...
  private:
    ...
    // subdomain, x, y, 2 wedges, quad points, 6 nodes, 3 components
    Kokkos::View< ScalarT*** [2][QuadRule::num_quad_points][6][3], grid::Layout > g1_;

    // subdomain, x, y, 2 wedges, quad points, 6 nodes, 3 components
    Kokkos::View< ScalarT*** [2][QuadRule::num_quad_points][6][3], grid::Layout > g2_;

    // subdomain, x, y, 2 wedges, quad points
    Kokkos::View< ScalarT*** [2][QuadRule::num_quad_points], grid::Layout > g3_;
  ...
  public:
    ...
    , g3_( "g3", grid.extent( 0 ), grid.extent( 1 ) - 1, grid.extent( 2 ) - 1 )
    {
    ...
    KOKKOS_INLINE_FUNCTION void operator()( const int local_subdomain_id, const int x_cell, const int y_cell ) const
    {
        ...
        for ( int wedge = 0; wedge < num_wedges_per_hex_cell; ++wedge )
        {
            ...
            for ( int q = 0; q < QuadRule::num_quad_points; ++q )
            {
                ...
                const ScalarT g3                                    = Kokkos::abs( det_b );
                g3_( local_subdomain_id, x_cell, y_cell, wedge, q ) = g3;
                const auto b_inv_t                                  = b.inv_transposed( det_b );

                for ( int i = 0; i < num_nodes_per_wedge; ++i )
                {
                    const ScalarT            shape_rad_i        = shape_rad( i, zeta );
                    dense::Vec< ScalarT, 3 > grad_shape_lat_vec = {
                        shape_rad_i * grad_shape_lat_xi< ScalarT >( i ),
                        shape_rad_i * grad_shape_lat_eta< ScalarT >( i ),
                        0.0 };
                    dense::Vec< ScalarT, 3 > g1                 = b_inv_t * grad_shape_lat_vec;
                    const ScalarT            shape_lat_i        = shape_lat( i, xi, eta );
                    dense::Vec< ScalarT, 3 > grad_shape_rad_vec = {
                        0.0, 0.0, shape_lat_i * grad_shape_rad< ScalarT >( i ) };
                    dense::Vec< ScalarT, 3 > g2 = b_inv_t * grad_shape_rad_vec;

                    for ( int d = 0; d < 3; ++d )
                    {
                        g1_( local_subdomain_id, x_cell, y_cell, wedge, q, i, d ) = g1( d );
                        g2_( local_subdomain_id, x_cell, y_cell, wedge, q, i, d ) = g2( d );
                    }
                }
            }
        }
    ...
    size_t team_shmem_size( int team_size ) const
    {
        ...
        return size_g1 + size_g2 + size_g3;
    }
    ...
    KOKKOS_INLINE_FUNCTION void operator()( const Team& team ) const
    {
        ...
        if ( team.team_rank() == 0 )
        {
            for ( int wedge = 0; wedge < num_wedges_per_hex_cell; ++wedge )
            {
                for ( int q = 0; q < N_q; ++q )
                {
                    shmem_g3[wedge * N_q + q] = g3_( local_subdomain_id, x_cell, y_cell, wedge, q );
                    for ( int k = 0; k < num_nodes_per_wedge; ++k )
                    {
                        for ( int d = 0; d < 3; ++d )
                        {
                            shmem_g1[d + 3 * ( k + num_nodes_per_wedge * ( q + N_q * wedge ) )] =
                                g1_( local_subdomain_id, x_cell, y_cell, wedge, q, k, d );
                            shmem_g2[d + 3 * ( k + num_nodes_per_wedge * ( q + N_q * wedge ) )] =
                                g2_( local_subdomain_id, x_cell, y_cell, wedge, q, k, d );
                        }
                    }
                }
            }
...
static_assert( linalg::OperatorLike< Laplace2S2< float, quadrature::QuadRuleWedge1Point< float > > > );
static_assert( linalg::OperatorLike< Laplace2S2< float, quadrature::QuadRuleWedge6Points< float > > > );
static_assert( linalg::OperatorLike< Laplace2S2< double, quadrature::QuadRuleWedge1Point< double > > > );
static_assert( linalg::OperatorLike< Laplace2S2< double, quadrature::QuadRuleWedge6Points< double > > > );
