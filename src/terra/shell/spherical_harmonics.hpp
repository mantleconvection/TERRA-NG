...
/// @brief Class providing methods for evaluation of Spherical Harmonics
///
/// @note If you are looking for unit-sphere **grids with interpolated spherical harmonics coefficients**, there are
///       helper functions that perform the evaluation and copy the resulting fields to the device (Kokkos::Views)
///       if required. See, e.g., \ref spherical_harmonics_coefficients_grid.
///
...
class SphericalHarmonicsTool
{
  ...
  private:
    template < typename T >
    static real_t real_c( T t )
    {
        return static_cast< real_t >( t );
    }

    template < typename T >
    static uint_t uint_c( T t )
    {
        return static_cast< uint_t >( t );
    }
    ...
    /// @brief Compute derivatives of associated Legendre functions
    ///
...
/// @brief Interpolates spherical harmonics coefficients of a specific order and degree into a grid (on the host),
/// then copies it to the device, then returns the device view.
///
/// @param degree_l spherical harmonics degree
/// @param order_m spherical harmonics order
/// @param coords_shell coordinates of the nodes of the unit sphere - compute with
///                     \ref terra::grid::shell::subdomain_unit_sphere_single_shell_coords
/// @return Scalar 3D grid with spherical harmonics coefficients on the sphere:
/// @code grid[local_subdomain_id][i][j] = coeff @endcode
template < std::floating_point ScalarTypeCoeff, std::floating_point ScalarTypeGrid >
grid::Grid3DDataScalar< ScalarTypeCoeff > spherical_harmonics_coefficients_grid(
    const int                                       degree_l,
    const int                                       order_m,
    const grid::Grid3DDataVec< ScalarTypeGrid, 3 >& coords_shell )
{
    grid::Grid3DDataScalar< ScalarTypeCoeff > sph_coeffs(
        "sph_coeffs_degree_l_" + std::to_string( degree_l ) + "_order_m_" + std::to_string( order_m ),
        coords_shell.extent( 0 ),
        coords_shell.extent( 1 ),
    ...
    // Plain loop since I am not sure if the sph tool can be used in parallel.
    for ( int local_subdomain_id = 0; local_subdomain_id < sph_coeffs.extent( 0 ); ++local_subdomain_id )
    {
        for ( int i = 0; i < sph_coeffs.extent( 1 ); ++i )
        {
