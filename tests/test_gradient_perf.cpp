/// @file  test_gradient_perf.cpp
/// @brief Wall-clock comparison of `Gradient` (old) vs `GradientKerngen` (fused,
///        team-based) at a realistic problem size. Mirror of test_divergence_perf.cpp.
...
int main( int argc, char** argv )
{
    ...
    VectorQ1Vec< ScalarType, 3 > dst_slow( "dst_slow", domain_fine,   mask_fine   );
    VectorQ1Vec< ScalarType, 3 > dst_fast( "dst_fast", domain_fine,   mask_fine   );
    ...
    std::cout << "Gradient perf  —  level_fine=" << level_fine
    ...
    for ( auto bc : bc_sets )
    {
        ...
        std::cout << "  old Gradient           : " << us_old  << " us/apply"
                  << "   (" << t_old  << " s total)" << std::endl;
        std::cout << "  new Gradient (slow)    : " << us_slow << " us/apply"
                  << "   (" << t_slow << " s total)"
                  << "   speedup vs old = " << ( us_old / us_slow ) << "x" << std::endl;
        std::cout << "  new Gradient (fast)    : " << us_fast << " us/apply"
                  << "   (" << t_fast << " s total)"
